...
update_cost <- function(params_i, prev_params, # these two are the only things that change each iteration
                        ...
                        error_func, cost_error_func, param_cost_func, min_cost_vec, min_params_mat
                     ) {
  # initializations
  ...
  for (new_index in obs_indexes) {
    ...
    error_i <- error_i + error_func(p_0 + T_1 - T_2 - obs_perf[[new_index]])
    j <- j + 1
    lower_index <- new_index + 1
    cost_ij <- cost_error_func(error_i, j) + params_cost_i
...
new_pred_perf <- function(init_params,
  ...
  if (type_error == "AE" || type_error == "MAE") {
    error_func <- abs
  }
  if (type_error %in% c("SSE", "MSE", "RMSE")) {
...
model_perf <- new_pred_perf(
  ...
  actual_perf,
  list("type" = "SSE"),
  list("type" = "lim_cost", "lambda" = 1000)
)
