...
TE_cost_func <- function(k, k_prime, tau, tau_alt, 
                         tau_i_sum_mat
                         TE, )
...
```{r}
min_params_2 <- function(params_init, # these two could change
                         tau_list, k_list,
                         training_load,
                         obs_perf, # these won't; they are data
                      # error_func, cost_error_func, param_cost_func, # these won't; they are specified by the user
                      # TE_cost_func, 
                      params_settings,
                      P_t, N_t) {
  ...
  ################################################
  ######## Making the tau matricies ##############
  ################################################
  
  M <- length(tau_list)
  tau_i_vec <- as.numeric(purrr::map(tau_list, ~exp(-1/.x)))
  tau_i_mat <- matrix(0, nrow = M, ncol = N)
  tau_i_sum_mat <- matrix(0, nrow = M, ncol = N)
  tau_i_mat[, 1] <- tau_i_vec
  tau_i_sum_mat[, 1] <- tau_i_vec
  for (i in 2:N) {
    tau_i_mat[, i] = tau_i_mat[, i-1]*tau_i_vec
    tau_i_sum_mat[, i] = tau_i_sum_mat[, i-1] + tau_i_mat[, i]
  }
  
  #################################################
  #### Computing the training effect matrices #####
  #################################################
  
  N_days <- length(training_load)
  TE_mat = matrix(0, M, N_days)
  
  ### maybe change based on what the first day should be ###
  TE_mat[, 1] <- tau_i_vec*training_load[[1]]
  for (i in 2:N_days){
    TE_mat[, i] = tau_i_vec*(TE_mat[, i-1] + training_load[[i]])
  }
  ...
  for (i in 1:length(obs_perf)) {
    ...
    if (is.na(obs_perf[[i]]) == FALSE) {
      k_1_prev <- params_prev[[2]]
      k_2_prev <- params_prev[[3]]
      coef_1_prev <- exp(-1/params_prev[[4]])
      coef_2_prev <- exp(-1/params_prev[[5]])
      
      coef_prev_1_vec <- rep(coef_1_prev, N)
      coef_prev_2_vec <- rep(coef_2_prev, N)
      coef_prev_1_sum <- rep(coef_1_prev, N)
      coef_prev_2_sum <- rep(coef_2_prev, N)
      
      
      
      for (i in 2:N) {
        coef_prev_1_vec[[i]] = coef_prev_1_vec[[i-1]] * coef_1_prev
        coef_prev_2_vec[[i]] = coef_prev_2_vec[[i-1]] * coef_2_prev
        coef_prev_1_sum[[i]] = coef_prev_1_sum[[i-1]] + coef_prev_1_sum^i
        coef_prev_2_sum[[i]] = coef_prev_2_sum[[i-1]] + coef_prev_2_sum^i
      }
      
      
      for (ii in 1:M) {            # iterating over tau_1
        for (jj in 1:M) {          # iterating over tau_2
          for (kk in 1:M_2) {      # iterating over k_1
            for (ll in 1:M_2) {    # iterating over k_2
              ...
              if (tau_1_i <= tau_2_i || k_1_i >= k_2_i) {
                next
              }
  ...
  # changes curr params to the update ones
  # endif  
  }
  
  # P_t <- exp(-1 / curr_params[[4]]) * (P_t + curr_params[[2]] * training_load[[i]])
  #   # training load index is  since i starts at 2
  # N_t <- exp(-1 / curr_params[[5]]) * (N_t + curr_params[[3]] * training_load[[i]])
  # perf_out[[i+1]] <- curr_params[[1]] +  P_t - N_t
  # matrix_params[i, ] <- as.numeric(curr_params)
  
    
  return(list("TE_mat" = TE_mat, 
              "tau_i_vec" = tau_i_vec))
...
test <- min_params_2(c(500,1,2,25,10), c(5:35), c(1:4), 
                     training_load = rep(100,100), 
                     obs_perf = actual_perf,
                     
                     params_settings = list("N" = 15, 
                                            "lambda_1" = 1, 
                                            "lambda_2" = 1,
                                            "training_level" = 100))
