...
## Literature Review
The Impulse-Response model was introduced in 1975 by Banister et al[@Banister:1975] to model athletic performance from training load. It has been used over the years to model athletic performance when the output can be easily quantified, such as swimming, cycling, power lifting, running, and the hammer throw. [@Mujika:1996; @Thomas:2008; @Busso:1994; @Busso:1997]. 
...
```
This model, with the given parameters captures a few phenomenon training with a constant training load. These are the initial gain from the workout (day 1 to 25), the plateau (day 25 to 38), and the taper (day 38 to 100). 
<!-- citation; original Busso-->

The previous example assumed that we had the optimal parameters for simplicity, but with real data, these would need to be estimated. The time-invariant impulse response parameter are obtained by finding the values minimizing the sum of the squared difference between the actual and predicted performance [@Banister:1975; @Clarke:2013; @Mujika:1996] (3 citations; original, clarke skibba, application). We can closely approximate the optimal parameters using this package's `optim_par` function, which amounts to using R's built in `optim` function. We apply this to our previous example below.  
...
(NEED TO MENTION WHAT RLS MEANS, CITE THE BOOK THE ALGORITHM IS BASED ON). The `RLS_predicted_performance` function is able to update our guess of what the parameter values are with each new data point using RLS. For instance, it performs better on the Winther data set than the Time-invariant model. 
