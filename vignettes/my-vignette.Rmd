...
Like all models this model is not perfect. The model works in some situations, but misses in others. Looking at the Winther data:
...
The time invariant model tracks the performance at the beginning of the graph, but then misses some of the points later.
Overall, it gets pretty close to the actual performances. This suggests that under some circumstances, the parameters of the athletes change over time. 
The main point of this package is to provide a model that can do so. 

# Time-varying Model: The `RLS_predicted_performance` Function

This function is the major improvement of this package. The `RLS_predicted_performance` function is able to update our guess of what the parameter values are with each new data point using Recursive Least Squares. For instance, it performs better on the Winther data set than the Time-invariant model. 

```{r, warning=FALSE}

winther_RLS_perf <- RLS_predicted_performance(
    training_load = winther_data[[2]],
    performance = winther_data[[3]], 
    p_0 = 250,
...
RLS_tib <- tibble::tibble(
  "day" = winther_data[[1]],
  "predicted_performance" =  winther_RLS_perf[[1]], #this function returns a list of things, the first one is the predicted performance
  "actual_performance" = winther_data[[3]] 
)


plot_winther_RLS <- ggplot(RLS_tib, aes(x=day, y=predicted_performance))+
  geom_line(color="blue")+
  geom_point(aes(y=actual_performance))
plot_winther_RLS
