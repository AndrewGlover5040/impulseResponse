...
load_all()
```
The goal is trying to figure out how well the time-varying model predicts/recovers the paramters of an athlete. 
...
prediction_with_alpha <- function(test, alpha, list_params){
  ...
  plots_list <- as.list(rep(NA,length(list_params)))
  for (i in plots_list) {
    plots_list[[i]] <- ggplot(parameter_tibble_pred, aes(x = day, y = ...)) +
      geom_line() +
...
prediction_with_alpha(test_1, .0001, list(tau_1))
...
noisy_perf_1 = purrr::map_dbl(non_noisy_perf_1, noise, sd = 1)
```
...
```{r}
params_list_traing = list(c(250,.1,.1,25,15), c(250,.5,.5,25,1)) # same as test 1
...
actual_param_tib_triangle <- actual_params_to_tib(test_triang)
